...
export default function AnalogClock({ size = 128 }) {
  ...
  const hand = (angle, len, w, color) => {
    ...
    return <line x1={tx} y1={ty} x2={x} y2={y} strokeWidth={w} strokeLinecap="round" style={{ stroke: color }} />;
  };
  ...
  const ticks = [...Array(12)].map((_, i) => {
    ...
    return <line key={i} x1={cx + r1 * Math.cos(a)} y1={cy + r1 * Math.sin(a)} x2={cx + r2 * Math.cos(a)} y2={cy + r2 * Math.sin(a)} strokeWidth={big ? 2.4 : 1.5} strokeLinecap="round" style={{ stroke: big ? T.ink : T.line }} />;
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7, flexShrink: 0 }}>
      <svg width={size} height={size}>
        <circle cx={cx} cy={cy} r={r} strokeWidth="2" style={{ fill: T.surface, stroke: T.line }} />
        <circle cx={cx} cy={cy} r={r - 1.5} fill="none" strokeWidth="1.2" style={{ stroke: T.grainSoft }} />
        {ticks}
        {hand(hourA, r * 0.5, 4, T.inkDeep)}
        {hand(minA, r * 0.72, 3, T.ink)}
        {hand(secA, r * 0.82, 1.4, T.grain)}
        <circle cx={cx} cy={cy} r="3.6" style={{ fill: T.ink }} />
        <circle cx={cx} cy={cy} r="1.5" style={{ fill: T.grain }} />
      </svg>
